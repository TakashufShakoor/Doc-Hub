        ...
        {appointments.map((item,index)=>(
          ...
          <div className='grid grid-cols-[1fr_2fr] gap-4 lg:flex  lg:gap-6 py-2 border-b items-center ' key={index}>
            ...
            <div className='flex flex-col gap-2  justify-end'>
             {!item.cancelled && !item.isCompleted && item.appointmentType === 'Virtual' &&  <button disabled={!item.payment} onClick={() => window.open(`http://localhost:5173/video_room/${item._id}`, '_blank')} className={`text-sm text-stone-500 text-center flex justify-center gap-2 sm:min-w-48 py-2 border items-center rounded  transition-all duration-300 ${item.payment ? 'bg-primary  hover:bg-primary/80' : 'bg-[#e0e0e0]'} `}><img className='w-6 '  src={assets.video_icon} alt="" /></button>} 
